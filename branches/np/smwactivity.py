...
def activityyearly(cursor):
	...
	rects = p.bar(np.arange(len(x)), y, color='#88aa33', align='center')
	p.legend()
	p.set_title('Activity by year')
	p.set_xlabel("Year")
	p.set_xticks(np.arange(len(x)))
	p.set_xticklabels([str(i) for i in x])
	p.set_ylabel('Edits')
	
	maxheight = max([rect.get_height() for rect in rects])
	for rect in rects:
		height = rect.get_height()
		p.text(rect.get_x()+rect.get_width()/2., height+(maxheight/25), str(height), ha='center', va='bottom')
