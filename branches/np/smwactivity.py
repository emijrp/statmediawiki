...
def activity(cursor, range='', title='', subtitle='', color='', xlabel='', timesplit=''):
    ...
    if range == 'global':
        ...
        a = cursor.execute("select strftime('%%%s', timestamp) as time, count(*) as count from revision where username='TXiKiBoT' group by time order by time asc" % (timesplit, ))
...
def activityall(cursor=None, range='', title=''):
    ...
    activityyearly(cursor=cursor, range=range, title=title)
