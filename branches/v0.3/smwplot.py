...
def printCombinedLinesGraph(title, filename, labels, headers, rows):
    xticsperiod = ""
    c = 0
    fecha = smwconfig.preferences["startDate"]
    fechaincremento=datetime.timedelta(days=1)
    ...
    c = 0
    for row in rows:
        plots.append(Gnuplot.PlotItems.Data(row, with_="lines", title=headers[c].encode(smwconfig.preferences['codification']))) #steps, filledcurve, ?
        c += 1
    gp(' set term png')
    gp(' set output "%s"' % (filename))
    gp.plot(*plots)
