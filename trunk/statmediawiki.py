...
def main():
	...
	#os.system("rm -r %s/*" % outputdir)
	sleep=0.1

	fechainicio=datetime.datetime(year=int(startdate.split("-")[0]), month=int(startdate.split("-")[1]), day=int(startdate.split("-")[2]), hour=0, minute=0, second=0)
	#fechainicio=datetime.datetime(year=2007, month=1, day=1, hour=0, minute=0, second=0)
	fechafin=datetime.datetime(year=int(enddate.split("-")[0]), month=int(enddate.split("-")[1]), day=int(enddate.split("-")[2]), hour=0, minute=0, second=0)
	...
	usersfile=open("%s/%s" % (outputdir, usersfilename), "w")
